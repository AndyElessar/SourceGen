    ...
    IEnvironmentProvider environmentProvider)
{
    ...
    /// <param name="maxApply">-m, How many matches should apply, 0 means unlimited.</param>
    /// <param name="isGenericAttribute">-ig, Whether to generate generic attribute syntax.</param>
    /// <param name="lifetime">-l, Lifetime for the service registration. Valid values: Singleton, Scoped, Transient.</param>
    /// <returns></returns>
        ...
        CancellationToken ct = default)
    {
        ...
        string normalizedLifetime = lifetime.Trim() switch
        {
            ...
            var value when value.Equals("Transient", StringComparison.OrdinalIgnoreCase) => "Transient",
            _ => string.Empty,
        };

        if(normalizedLifetime.Length == 0)
        {
            logger.ZLogError($"-l|--lifetime must be one of: Singleton, Scoped, Transient. Provided: {lifetime}");
            return;
        }
        ...
        foreach(var (b, cs) in collected)
        {
            ...
            if(isGenericAttribute)
                sb.AppendLine($"[assembly: IocRegisterDefaults<{b}>(ServiceLifetime.{normalizedLifetime}, ImplementationTypes = [{typeofList}])]");
            else
                sb.AppendLine($"[assembly: IocRegisterDefaults(typeof({b}), ServiceLifetime.{normalizedLifetime}, ImplementationTypes = [{typeofList}])]");
        }
        sb.AppendLine();
