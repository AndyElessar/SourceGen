...
- SGIOC006 - Warning - Design - Nested OpenGeneric Detected
    ...
    - The source generator will auto-generate closed generic registrations when closed generic types are used in constructor parameters or `GetService`/`GetRequiredService` calls.
    - Example (Warning): `class Handler<T> : IHandler<Wrapper<T>>` - `Wrapper<T>` is a generic type with unbound `T` used as type argument.
...
- SGIOC011 - Warning - Usage - Duplicated Attribute Usage
