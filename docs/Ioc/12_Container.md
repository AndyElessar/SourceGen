...
> [!TIP]
> For most .NET applications, the simplest way to integrate the generated container is to use it with `MS.E.DI`, you can call `new Container().CreateServiceProvider()` to get a container has `MS.E.DI` as fallback `IServiceProvider`.
