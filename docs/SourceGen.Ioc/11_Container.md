...
partial class AppContainer :
    ...
    IAsyncDisposable
{
    private global::SystemClock _systemClock;
    ...
    {
        // resolver lookup generated here
    }
}
```
...
|`IServiceProviderFactory<IServiceCollection>`|Generated only when `ResolveIServiceCollection = true` and DI package support is available; used to build an external fallback provider.|
...
[
    // registrations without matching tags are excluded
    new(new ServiceIdentifier(typeof(global::IMessageBus), KeyedService.AnyKey), static c => c.GetMessageBus()),
];
```
