...
## Wrapper Types with Open Generics

Wrapper dependencies can also trigger closed generic discovery.
...
### `IDictionary<TKey, TValue>` and `KeyValuePair<TKey, TValue>`

For keyed services, dictionary wrappers are built from generated `KeyValuePair<TKey, TValue>` service entries.
